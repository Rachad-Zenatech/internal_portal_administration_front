...
export const ChatMessageRenderer: React.FC<ChatMessageRendererProps> = ({
  ...
  const handleLinkClick = (url: string, e: React.MouseEvent) => {
    ...
    if (url.startsWith("http://") || url.startsWith("https://")) {
      ...
      let targetUrl = url;
      // Normalize /purchasing/:id to /purchasing/requests/:id
      const prMatch = url.match(/^\/purchasing\/(\d+)$/);
      if (prMatch) {
        targetUrl = `/purchasing/requests/${prMatch[1]}`;
      }
      navigate(targetUrl);
      if (onNavigate) {
        onNavigate();
        ...
        if (isInternalPR) {
          tokens.push(
            <button
              ...
              className="inline-flex items-center gap-1 px-2 py-0.5 mx-0.5 my-0.5 rounded-md font-medium text-xs bg-blue-50 hover:bg-blue-100 dark:bg-blue-950/60 dark:hover:bg-blue-900/80 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-800 transition-colors shadow-xs cursor-pointer group"
            >
      ...
      {parsedBlocks.map((block, idx) => {
        ...
        if (block.type === "card" && block.cardData) {
          ...
          return (
            <div
              key={idx}
              className="my-2.5 p-3 rounded-xl border border-blue-200 dark:border-blue-900/60 bg-gradient-to-br from-blue-50/80 via-indigo-50/30 to-background dark:from-blue-950/40 dark:via-zinc-900/60 dark:to-zinc-900 shadow-xs hover:shadow-md transition-all group"
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <Button
                  size="sm"
                  onClick={(e) => handleLinkClick(url, e)}
                  className="h-8 px-3 text-xs font-medium bg-blue-600 hover:bg-blue-700 text-white shadow-xs gap-1.5 shrink-0 group-hover:translate-x-0.5 transition-transform cursor-pointer"
                >
