...
import {
  useMasterLocations,
  useCreateMasterLocation,
  useUpdateMasterLocation,
  useDeleteMasterLocation,
  useImportMasterLocations,
...
export default function Locations() {
  ...
  const { data, isLoading, isRefetching, refetch } = useMasterLocations();
  const createMutation = useCreateMasterLocation();
  const updateMutation = useUpdateMasterLocation();
  const deleteMutation = useDeleteMasterLocation();
  const importMutation = useImportMasterLocations();

  const locations = data?.items || [];
  const totalCount = data?.total || locations.length;
  ...
  const filteredLocations = useMemo(() => {
    ...
    return locations.filter(
      (loc) =>
        ...
        (loc.raw_name && loc.raw_name.toLowerCase().includes(term)) ||
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-zinc-800 pb-5">
        <div>
          <div className="flex items-center gap-2.5">
            <div className="p-2 bg-sky-100 dark:bg-sky-950/70 rounded-xl text-sky-700 dark:text-sky-300">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Manage and standardize company locations, branches, and geographic sites.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <Button
            ...
            className="h-9 gap-1.5 text-xs font-semibold bg-sky-600 hover:bg-sky-700 text-white shadow-xs"
          >
      ...
      <div className="relative">
        ...
        <Input
          placeholder="Search locations by standardized name, raw text, or description..."
          value={searchTerm}
        ...
        <Table>
          <TableHeader className="bg-slate-50 dark:bg-zinc-800/60">
            <TableRow className="border-slate-200 dark:border-zinc-800 text-[11px] font-semibold text-slate-600 dark:text-zinc-400 uppercase tracking-wider">
              <TableHead className="w-[35%] py-3">Location Name (Standardized)</TableHead>
            ...
            {isLoading ? (
              ...
              <TableRow>
                <TableCell colSpan={5} className="py-12 text-center text-muted-foreground">
                  <MapPin className="h-8 w-8 mx-auto text-slate-300 dark:text-zinc-600 mb-2" />
                  <p className="font-medium text-sm">No locations found</p>
                  <p className="text-xs mt-1">
                    {searchTerm ? "Try adjusting your search terms" : "Click 'Add Location' or 'Import' to add records."}
                  </p>
                </TableCell>
              </TableRow>
            ) : (
      ...
      <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Enter location name. It will be automatically formatted per company standards.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              ...
              <Input
                placeholder="e.g. Cardinal Civil Resources:Manassas, VA or Vancouver, BC"
                value={newName}
                onChange={(e) => setNewName(e.target.value)}
                className="text-sm font-medium"
              />
              {newName && (
                <p className="text-[11px] text-sky-700 dark:text-sky-300 font-mono bg-sky-50 dark:bg-sky-950/50 p-2 rounded border border-sky-200 dark:border-sky-900">
            ...
            <div className="space-y-1.5">
              ...
              <Textarea
                rows={2}
                placeholder="Additional notes or address details..."
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <label htmlFor="editActive" className="text-xs font-medium cursor-pointer">
      ...
      <Dialog open={importDialogOpen} onOpenChange={setImportDialogOpen}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="rounded-lg bg-slate-50 dark:bg-zinc-800/60 p-3 text-xs text-slate-600 dark:text-zinc-300 space-y-1 border border-slate-200 dark:border-zinc-800">
              ...
              <ul className="list-disc pl-4 space-y-0.5 text-[11px] text-muted-foreground">
                <li>Company locations formatted as <code>Company (Location)</code></li>
                <li>Standalone locations and countries preserved</li>
                <li>Extra spaces and technical characters removed</li>
              </ul>
            </div>
          </div>
          ...
          <DialogFooter>
            ...
            >
              {importMutation.isPending ? "Importing & Standardizing..." : "Import Locations"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
